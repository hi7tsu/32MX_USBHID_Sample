...
#if !defined(USB_HAL_PIC32MX_H)
#define USB_HAL_PIC32MX_H
...
/* 
#if defined(__32MX210F016B__) || defined(__32MX210F016C__) || defined(__32MX210F016D__)
    //Number of IECx registers implemented in the microcontroller (varies from device to device, make sure this is set correctly for the intended CPU)
    #define DEVICE_SPECIFIC_IEC_REGISTER_COUNT  2   
...
//Note: Low speed device mode is only supported on certain devices.
#define USB_FULL_SPEED                  0x00
#define USB_LOW_SPEED                   0x08
...
#define USBRESUMEIFBitNum               5
...
#if ((__PIC32_FEATURE_SET__ >= 100) && (__PIC32_FEATURE_SET__ <= 499))
    // 1xx, 2xx, 3xx, 4xx
    #define USBInterruptFlag        IFS1bits.USBIF
    #define USBInterruptEnableBit   IEC1bits.USBIE
    #define USBIPLBits              IPC7bits.USBIP
    #define USBISLBits              IPC7bits.USBIS
#elif ((__PIC32_FEATURE_SET__ >= 500) && (__PIC32_FEATURE_SET__ <= 799))
    // 5xx, 6xx, 7xx
    #define USBInterruptFlag        IFS1bits.USBIF
    #define USBInterruptEnableBit   IEC1bits.USBIE
    #define USBIPLBits              IPC11bits.USBIP
    #define USBISLBits              IPC11bits.USBIS
#else
    // unknown
    #error "Unsupported feature set detected: Check usb_hal_pic32mx.h"
#endif
    ...
    #define USBMaskInterrupts() {USBInterruptEnableBit = 0;}
    #define USBUnmaskInterrupts() {USBInterruptEnableBit = 1;}
#else
    #define USBEnableInterrupts()
...
#define USBDisableInterrupts() {USBInterruptEnableBit = 0;}
#define USBClearUSBInterrupt() {USBInterruptFlag = 0;}
...
// Physical Addr to Virtual Addr(KSEG1)
#ifndef PA_TO_KVA1
